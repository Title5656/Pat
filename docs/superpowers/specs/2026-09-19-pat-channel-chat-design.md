...
- ถ้า Gemini เรียกไม่สำเร็จ ให้ตอบ fallback พร้อม error ที่ผ่านการตัดความยาวและปิดบัง key/token
- เก็บ error object ฉบับเต็มไว้ใน server logs และแสดง safety block reason เมื่อ Gemini ส่งกลับมา
